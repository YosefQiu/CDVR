...
public:
    ...
    // 构建KDTree
    bool buildTree(const std::vector<SparsePoint>& inputPoints);
    bool buildTree(const SparsePoint* points, size_t numPoints);
    
    // K近邻查询
    template<int K>
    bool knnSearch(const SparsePoint& queryPoint, float searchRadius, 
...
private:
    ...
    std::vector<SparsePoint> m_originalPoints;   // 原始输入点
    kdTree::box_t<kdTree::float2> m_worldBounds; // 世界边界
    size_t m_pointCount;
    bool m_isBuilt;
    
    // 辅助函数
    kdTree::float2 sparseToKDTree(const SparsePoint& point) const;
    GPUPoint sparseToGPU(const SparsePoint& point) const;
    SparsePoint kdtreeToSparse(const kdTree::float2& point, int originalIndex) const;
};